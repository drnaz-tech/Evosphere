# EvoSphere Deployment Guide

This guide covers deploying EvoSphere to three popular static hosting platforms: **Vercel**, **Netlify**, and **GitHub Pages**.

> EvoSphere is a client-side single-page application (SPA) built with Vite, React, TypeScript, and PixiJS. It requires no server-side runtime and can be deployed as a static bundle.
...
- Node.js ≥ 20
- npm ≥ 10 (or pnpm ≥ 8)
- A Git repository with the EvoSphere source code pushed
...
EvoSphere does not require backend API keys for core functionality. If you add Supabase or other integrations later, add environment variables in **Project Settings → Environment Variables**.
...
3. Authorize Netlify and select your EvoSphere repository
...
EvoSphere currently uses these environment variables (all optional for basic deployment):
...
*Ready to share EvoSphere with the world? Pick a platform above and deploy in minutes.*
